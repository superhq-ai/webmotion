...
- **Audio packs the same way.** Each `<w-audio>` clip renders as a block at its timeline position. Clips that overlap in time, a music bed under effects for example, go to separate lanes; clips that do not overlap share one.
