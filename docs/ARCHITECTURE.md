...
- **Live-canvas layers.** An entity that exposes `wmLiveCanvas()` (`<w-model>`, `<w-transition>`, `<w-video>`) is captured per frame with `createImageBitmap` off its own canvas instead of being DOM-rasterized, and settles first through `wmAwaitFrame()`. This is the fast path for anything that draws its own pixels every frame: a fullpage dither cut built this way is one live layer, where the same cut built from hundreds of animated cells re-serializes the frame each frame (see [TRANSITIONS.md](./TRANSITIONS.md)).
